...
SET /A seq=seq+1
docker build -f .\docker_application_build --tag tarantula.asset:%version% --build-arg app=asset --build-arg h=%host% --build-arg n=asset%id% --build-arg i=%seq% --build-arg g=%grp% .
SET /A seq=seq+1
